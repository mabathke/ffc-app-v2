...
def home():
    ...
    return render_template('dashboard.html', title='Home', catches=catches, catches_per_user= catches_per_user)
...
def fangmeldung():
    ...
    if form.validate_on_submit():
        ...
        # Calculate avg_length before adding the new catch
        total_catches = Catch.query.filter_by(fish_id=selected_fish.id).count()
        total_length = db.session.query(func.sum(Catch.length)).filter_by(fish_id=selected_fish.id).scalar() or 0

        if total_catches > 0:
            avg_length = total_length / total_catches
        else:
            # If there are no previous catches, use the midpoint of lower and upper bounds
            avg_length = (lower_bound + upper_bound) / 2
        ...
        # Update the average length including the new catch
        total_catches += 1
        total_length += length
        selected_fish.avg_length = total_length / total_catches if total_catches > 0 else 0
